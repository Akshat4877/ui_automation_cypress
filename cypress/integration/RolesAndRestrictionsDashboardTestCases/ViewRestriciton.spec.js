...
describe("Roles And Restrication For Dashboard(View)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Dashboard(View)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Apply View Restriction', function () {
        ...
        cy.xpath('//*[text() ="Dashboard"]')
            .within(($Dashboard) => {
                cy.xpath('//*[text() ="Dashboard"]').scrollIntoView({ force: true })
                cy.wait(1000)
                //Click on View
                cy.xpath("//*[@class='v-chip__content' and contains(text(),'Workflow Tab')]").eq(13)
                .click({ force: true })
                cy.wait(2000)
    ...
    it.only('Validate (View)Restriciton for Dashboard Tab',function(){
        //Dashboard Tab should not be visible/exist
        cy.wait(1000);
        //should not click because the Dashboard tab should not ne there
