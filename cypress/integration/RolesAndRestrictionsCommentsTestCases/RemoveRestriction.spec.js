...
describe("Roles And Restrication For Comments(Remove)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Comments(Remove)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Apply Remove Restriction', function () {
        ...
        cy.xpath('//*[text() ="Comments"]')
            .within(($Comments) => {
                ...
                cy.wait(1000)
                //Click on Remove
                cy.xpath("//*[@class='v-chip__content' and contains(text(),'Remove')]").eq(8)
                    .click({ force: true })
                cy.wait(2000)
    ...
    it.only('Add A Comment',function(){
      ...
      .type('This is the comment for remove restriction');
      cy.wait(1000)
      //Click on save,
    ...
    it.only('Validate (Remove)Restriciton in Comments Tab',function(){
      //remove validation msg
      cy.contains(' You are not permitted to remove any comments for '+'"'+this.KitTypeName.KitName3+'"'+' please contact your administrator to remove this restriction ')
      .should('be.visible')
    })
