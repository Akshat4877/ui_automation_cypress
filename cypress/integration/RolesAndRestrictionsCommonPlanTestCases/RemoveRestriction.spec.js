...
describe("Roles And Restrication For Common Plans(Remove)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Common Plans(Remove)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Open Common Plans Tab',function(){
    ...
    it.only("Create A Common Plan", function () {
        ...
        //Creating Plan
        cy.get('[placeholder="Plan Name"]').type('Role Restriction Plan');
        ...
        cy.contains('Role Restriction Plan').should("be.visible");
        cy.log("Common Plan Created");
        cy.wait(1000);
    });

     it.only('Validate (Remove)Restriciton in Common Plans Tab',function(){
       //Click on Delete icon for Common Plans
       cy.get(".space-planner-card--action-icon path").eq(0)
      .click({ force: true });
      cy.contains(' You do not have permission to remove any Common Plan for '+'"'+this.KitTypeName.KitName3+'"'+' Please contact your administrator for help ')
       .should('be.visible')  
    })
})