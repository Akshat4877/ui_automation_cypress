...
describe("Roles And Restrication For Time Entries(Create)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Time Entries(Create)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Apply Create Restriction', function () {
        ...
        cy.xpath('//*[text() ="Time"]')
            .within(($Details) => {
                cy.xpath('//*[text() ="Time"]').scrollIntoView()
                cy.wait(1000)
                //Click on Create
                cy.xpath("//*[@class='v-chip__content' and contains(text(),'Create')]").eq(10)
                    .click({ force: true })
                cy.wait(2000)
    ...
    cy.contains(' You are not permitted to create any time entry for '+'"'+this.KitTypeName.KitName3+'"'+' please contact your administrator to remove this restriction ')
    .should('be.visible')
    })
