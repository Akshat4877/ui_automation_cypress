...
describe("Roles And Restrication For Account Filters(Modify All)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Account Filters(Modify All)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Apply Modify All Restriction', function () {
        ...
        cy.xpath('//*[text() ="Account Filters"]')
            .within(($AccountFilters) => {
                ...
                cy.wait(1000)
                //Click on Modify All
                cy.xpath("//*[@class='v-chip__content' and contains(text(),'Modify All')]").eq(12)
                    .click({ force: true })
                cy.wait(2000)
    ...
    it.only('Create Account Filter for Url',function(){
        ...
        .type('Restriction Account Filter')
        cy.wait(2000)
      })
    ...
    it.only('Apply Modify All Action for Account Filter',function(){
        ...
        cy.contains('Restriction Account Filter ').click({force:true})
        cy.wait(2000)
         //Click on Filter Drop down
        cy.xpath("//*[text() = 'arrow_drop_down']").eq(0)
        .click({force:true});
        cy.wait(2000)
        //Click on save Filter form drop down
        cy.xpath('//*[text()="Save filter"]').last().click({force:true})
        cy.wait(2000)
    })

    it.only('Validate (Modify All)Restriciton for Account Filter',function(){
