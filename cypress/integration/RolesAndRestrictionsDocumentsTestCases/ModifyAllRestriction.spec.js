...
describe("Roles And Restrication For Documents(ModifyAll)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Documents(ModifyAll)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Apply ModifyAll Restriction', function () {
        ...
        cy.xpath('//*[text() ="Documents"]')
            .within(($Documents) => {
                ...
                cy.wait(1000)
                //Click on ModifyAll
                cy.xpath("//*[@class='v-chip__content' and contains(text(),'Modify All')]").eq(7)
                    .click({ force: true })
                cy.wait(2000)
    ...
    it.only('Validate (ModifyAll)Restriciton in Files Tab',function(){
      cy.wait(1000)
      //Click on Internally Viewable for file
      cy.xpath("//*[@class='v-chip__content' and contains(text(),' Internally Viewable ')]")
      .eq(0)
      .click({ force: true });
      cy.contains("Oops! Can't update file ")
      .should('be.visible')
      cy.contains(' You are not permitted to change the modify status on the file, please contact your administrator ')
      .should('be.visible')
    })
