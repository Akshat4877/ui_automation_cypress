...
describe("Roles And Restrication For Group Assignment(Remove)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Group(Remove)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Apply Remove Restriction', function () {
        ...
        cy.xpath('//*[text() ="Group Assignment"]')
            .within(($Group) => {
                ...
                cy.wait(1000)
                //Click on Remove
                cy.xpath("//*[@class='v-chip__content' and contains(text(),'Remove')]").eq(9)
                    .click({ force: true })
                cy.wait(2000)
    ...
    it.only('Validate (Remove)Restriciton in Group Tab',function(){
    
     //Click on delete icon of Grops
     cy.get('.group:nth-child(1) > .v-list-item__action path')
     .click({force: true});
     cy.contains(' You are not permitted to remove any group for '+'"'+this.KitTypeName.KitName3+'"'+' please contact your administrator to remove this restriction ')
     .should('be.visible')
