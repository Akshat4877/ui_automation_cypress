...
describe("Roles And Restrication For Group Assignment(Create)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Group(Create)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Apply Create Restriction', function () {
        ...
        cy.xpath('//*[text() ="Group Assignment"]')
            .within(($Group) => {
                ...
                cy.wait(1000)
                //Click on Create
                cy.xpath("//*[@class='v-chip__content' and contains(text(),'Create')]").eq(8)
                    .click({ force: true })
                cy.wait(2000)
    ...
    it.only('Validate (Create)Restriciton in Group Tab',function(){
    
     //Click on Add for Grops
     cy.get(".details-wrapper > .col > .ml-0 > .v-btn__content")
     .click({force: true});
     cy.contains(' You are not permitted to create any group for '+'"'+this.KitTypeName.KitName3+'"'+' please contact your administrator to remove this restriction ')
     .should('be.visible')
