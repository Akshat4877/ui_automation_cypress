...
describe("Roles And Restrication For Group Assignment(View)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Group(View)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Apply View Restriction', function () {
        ...
        cy.xpath('//*[text() ="Group Assignment"]')
            .within(($Group) => {
                cy.xpath('//*[text() ="Group Assignment"]').scrollIntoView({ force: true })
                cy.wait(1000)
                //Click on View
                cy.xpath("//*[@class='v-chip__content' and contains(text(),'View')]").eq(10)
                    .click({ force: true })
                cy.wait(2000)
    ...
    it.only('Validate (View)Restriciton for Group Tab',function(){
        //Group Tab should not be visible/exist
        cy.wait(1000);
        //should not click because the Group tab should not there
        cy.contains(' Groups ').should('not.exist') 
        cy.log('Group tab is not exist')
        cy.wait(1000)
    })
