...
describe("Create Active schedule for Kit Item through Add button in calendar", function () {
  ...
  this.beforeEach("KitType Data", function () {
    ...
    cy.fixture("SanityPackTestData2/Scheduler").then(function (KitDataEle) {
      this.NewKitItemData = KitDataEle;
    });

    // cy.fixture("SanityPackTestData(Prod)/Scheduler(Prod)").then(function (
    //   KitDataEle
    // ) {
    //   this.NewKitItemData = KitDataEle;
    // });
  ...
  it.only("Navigate to kit item list View(Left Panel) to open Calendar", function () {
    ...
    cy.get('.sections-header__checkbox-label')
      .click({ force: true })
    cy.wait(2000)
    //click on 
    cy.get('.toolbar-action:nth-child(2) path').click({ force: true });
    cy.wait(10000)
