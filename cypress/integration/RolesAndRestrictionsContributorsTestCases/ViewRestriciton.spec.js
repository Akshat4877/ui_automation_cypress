...
describe("Roles And Restrication For Contributors(View)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Contributors(View)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Apply View Restriction', function () {
        ...
        cy.xpath('//*[text() ="Contributors"]')
            .within(($Details) => {
                ...
                cy.wait(1000)
                //Click on View
                cy.xpath("//*[@class='v-chip__content' and contains(text(),'View')]").eq(5)
                    .click({ force: true })
                cy.wait(2000)
    ...
    it.only('Validate (View)Restriciton for Contributors Tab',function(){
        //Contributors Tab should not be visible/exist
        cy.wait(1000);
        //should not click because the Contributors tab should not ne there
        cy.contains(' Contributors ').should('not.exist') 
    })

    it.only('Contributors Tab validation',function(){
        //Add button in contributors should not visible coz Contirbutors tab should not exit
        cy.get(".addBtn:nth-child(2) > .v-btn__content").should('not.exist')
    })
