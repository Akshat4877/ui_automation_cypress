...
describe("Roles And Restrication For Report(View)", function () {
    ...
    it.only('Select kit type to Configure Restriction For Report(View)', function () {
        const RoleRestr = new RolesAndRestrictionsPage();
        cy.wait(2000)
    ...
    it.only('Apply View Restriction', function () {
        ...
        cy.xpath('//*[text() ="Report"]')
            .within(($Details) => {
                cy.xpath('//*[text() ="Report"]').scrollIntoView({ force: true })
                cy.wait(1000)
                //Click on View
                cy.xpath("//*[@class='v-chip__content' and contains(text(),'View')]").eq(9)
                    .click({ force: true })
                cy.wait(2000)
    ...
    it.only('Validate (View)Restriciton for Report Tab',function(){
        //Report Tab should not be visible/exist
        cy.wait(1000);
        //should not click because the Report tab should not ne there
